...
Using with: syntax, this class takes care of getting the table created, allowing custom resource write to store data and retrieve data.
"""
...
if TYPE_CHECKING:
    from mypy_boto3_dynamodb.service_resource import DynamoDBServiceResource as DynamoDBServiceResource
else:
...
class ResourceCreate(_ResourceBase):
    ...
    #     #AUTOMATIC CHECKING OF AlreadyEXISTS done by DynamoDB code when we have a primary identifier ahead of time. IE a user parameter
    #     created_resource = self.action()
    #     if created_resource:
    #        DB.set_resource_created(primary_identifier=self._callback_context["working_model"].ReadOnlyIdentifier,
    #                                working_model=self._callback_context["working_model"])
    ...
    def duplicate_primary_identifier(self) -> None:
        if self._db_item_exists():
            raise cloudformation_cli_python_lib.exceptions.\
                AlreadyExists(type_name=self._type_name,
    ...
    def __exit__(self,
        ...
        # If success but set_resource_created was not called - Do nothing in Dynamo - resource already existed - statemachine execution
        ...
        if exception_type is None:
            ...
            if not self._set_resource_created_called:
                ...
                self._db_item_insert_without_overwrite(typing.cast(BaseModel, self._current_model))
        else:
