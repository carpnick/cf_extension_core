...
from cf_extension_core.dynamo_table_creator import DynamoTableCreator as _DynamoTableCreator

from cloudformation_cli_python_lib.interface import BaseResourceHandlerRequest as _BaseResourceHandlerRequest, BaseModel as _BaseModel
...
class ResourceBase:
    ...
    def __init__(self,
        ...
        _DynamoTableCreator(self._db_resource).create_standard_table()
    ...
    class ResourceData:
        ...
        @staticmethod
        def _model_from_string(modelstr: str, class_type: Type[T]) -> Any:
            try:
    ...
    def _get_primary_identifier(self) -> str:
        if self._primary_identifier == None:
    ...
    def _not_found_check(self) -> None:
        if not self._db_item_exists():
            raise exceptions.\
                NotFound(type_name=self._type_name,
                         identifier=self._get_primary_identifier())
        ...
        try:
            the_table.put_item(
                ...
                ExpressionAttributeNames={
                    '#pk': constants.DynamoDBValues.PARTITION_KEY
                },
            )
            ...
            raise Exception("Attempting to create resource record failed in dynamo, please contact CLOUD team for help") from ex
    ...
    def _db_item_exists(self) -> bool:
        ...
        if myitem is None:
            ...
            return True
    def _db_item_get_model(self, model_type: Type[T]) -> Any:
    ...
    def _db_item_list_primary_identifiers_for_cr_type(self):
        ...
        scan_output = self._db_resource.meta.client.scan(
            ...
            ExpressionAttributeNames={
                '#tn': constants.RowColumnNames.TYPE_NAME
            },
            ExpressionAttributeValues={
                ':tn': self._type_name
            },
        ...
        if "LastEvaluatedKey" in scan_output:
            ...
            while(last_evaluated_key_data !=None):
    ...
    ##### DELETE Requests######
    def _db_item_delete(self, best_effort: bool = False) -> None:

        logger.info("Deleting item with best effort set to : %s. Primary Key: %s", str(best_effort), self._get_primary_identifier())
        ...
        try:
            ...
            if best_effort:
                ...
                raise Exception("Attempting to delete resource record failed in dynamo, please contact CLOUD team for help") from ex
