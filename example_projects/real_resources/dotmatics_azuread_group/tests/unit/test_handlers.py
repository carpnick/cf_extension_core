...
from dotmatics_azuread_group.handlers import create_handler, update_handler, delete_handler, read_handler
...
def get_json() -> Any:
    with open(os.getcwd() + os.sep + "dotmatics-azuread-group.json", "r") as f:
        return json.load(f)
...
def test_timeouts_for_each_entrypoint_match_schema(mocker: MockFixture) -> None:
    ...
    rh = mocker.MagicMock()
    mocker.patch(target="dotmatics_azuread_group.handlers.ReadHandler", new=rh)

    uh = mocker.MagicMock()
    mocker.patch(target="dotmatics_azuread_group.handlers.UpdateHandler", new=uh)

    dh = mocker.MagicMock()
    mocker.patch(target="dotmatics_azuread_group.handlers.DeleteHandler", new=dh)

    ch = mocker.MagicMock()
    mocker.patch(target="dotmatics_azuread_group.handlers.CreateHandler", new=ch)
